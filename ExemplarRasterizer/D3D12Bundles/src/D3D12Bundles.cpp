...
void D3D12Bundles::LoadPipeline()
{
	...
	m_Rasterizer = std::unique_ptr<Rasterizer>(
		new Rasterizer(m_device.Get(), m_width, m_height, DXGI_FORMAT_R8G8B8A8_UNORM));
	...
	// Create descriptor heaps.
	{
		...
		BuildRootSignature(featureData, error);
		//Create compute root signature
		BuildRasterizerRootSignature(featureData, error);

		BuildDescriptorHeaps();


		//UINT mCbvSrvUavDescriptorSize = m_device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

		//m_Rasterizer->AssignDescriptorsStart(
		//	CD3DX12_CPU_DESCRIPTOR_HANDLE(m_cbvSrvHeap->GetCPUDescriptorHandleForHeapStart()),
		//	CD3DX12_GPU_DESCRIPTOR_HANDLE(m_cbvSrvHeap->GetGPUDescriptorHandleForHeapStart()),
...
void D3D12Bundles::BuildRootSignature(const D3D12_FEATURE_DATA_ROOT_SIGNATURE& featureData, ComPtr<ID3DBlob>& error)
{
...
void D3D12Bundles::BuildRasterizerRootSignature(const D3D12_FEATURE_DATA_ROOT_SIGNATURE& featureData, ComPtr<ID3DBlob>& error)
{
	//old one 
	//// Create compute signature.
	//CD3DX12_DESCRIPTOR_RANGE1 rangesCompute[2];
	//rangesCompute[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 0, 0, D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC_WHILE_SET_AT_EXECUTE);
	//rangesCompute[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 0, 0, D3D12_DESCRIPTOR_RANGE_FLAG_DATA_VOLATILE);

	////CD3DX12_ROOT_PARAMETER slotRootParameter[4];
	//CD3DX12_ROOT_PARAMETER1 slotRootParameter[2];
	//slotRootParameter[0].InitAsDescriptorTable(2, rangesCompute);
	...
	CD3DX12_DESCRIPTOR_RANGE1 srvTable1;
	srvTable1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0);

	CD3DX12_DESCRIPTOR_RANGE1 srvTable2;
	...
	ComPtr<ID3DBlob> signatureCompute;
	//ComPtr<ID3DBlob> error;
	ThrowIfFailed(D3DX12SerializeVersionedRootSignature(&rootSigDescCompute, featureData.HighestVersion, &signatureCompute, &error));
	ThrowIfFailed(m_device->CreateRootSignature(0, signatureCompute->GetBufferPointer(), signatureCompute->GetBufferSize(), IID_PPV_ARGS(&m_rootSignatureCompute)));
	NAME_D3D12_OBJECT(m_rootSignatureCompute);
...
// buffer view (CBV) descriptor heap.
	...
	//Lower descriptors for on city only
	cbvSrvHeapDesc.NumDescriptors =
		FrameCount // FrameCount frames * CityRowCount * CityColumnCount.
		+ 1 + RasterizerDescriptorCount;                                            // + 1 for the SRV.
	...
	m_Rasterizer->AssignDescriptorsStart(
		CD3DX12_CPU_DESCRIPTOR_HANDLE(m_cbvSrvHeap->GetCPUDescriptorHandleForHeapStart(), /*RasterizerDescriptorCount*/FrameCount, mCbvSrvUavDescriptorSize),
		CD3DX12_GPU_DESCRIPTOR_HANDLE(m_cbvSrvHeap->GetGPUDescriptorHandleForHeapStart(), /*RasterizerDescriptorCount*/FrameCount, mCbvSrvUavDescriptorSize),
		mCbvSrvUavDescriptorSize);
}
...
void D3D12Bundles::LoadAssets()
{
	...
	// Create the pipeline state, which includes loading shaders.
	{
		...
		//Added
		ThrowIfFailed(ReadDataFromFile(GetAssetFullPath(L"rasterizerCompute.cso").c_str(), &pComputeShaderData, &computeShaderDataLength));
		...
		//ADDED
		//Create Compute PSO
		D3D12_COMPUTE_PIPELINE_STATE_DESC psoCompute = {};
		psoCompute.pRootSignature = m_rootSignatureCompute.Get();
		psoCompute.CS = CD3DX12_SHADER_BYTECODE(pComputeShaderData, computeShaderDataLength);
		psoCompute.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;

		//TODO might need third param
		ThrowIfFailed(m_device->CreateComputePipelineState(&psoCompute, IID_PPV_ARGS(&m_pipelineStateCompute)));

		delete pVertexShaderData;
		delete pPixelShaderData1;
		delete pPixelShaderData2;
		delete pComputeShaderData;
	}
	...
	//CreateQuadVertexAndIndexBuffer();

		//create quad vertex buffer

	ComPtr<ID3D12Resource> vertexBufferQuadsUploadHeap;
	ComPtr<ID3D12Resource> indexBufferQuadsUploadHeap;
	//if(UseCompiuteRasterizer)
	{

		static std::array<SampleAssets::VertexCustom, 4> vertices =
		{
			SampleAssets::VertexCustom({ XMFLOAT3(-10.0f, -10.0f, -10.0f), XMFLOAT4(Colors::White) }),
			SampleAssets::VertexCustom({ XMFLOAT3(-10.0f, +10.0f, -10.0f), XMFLOAT4(Colors::Black) }),
			SampleAssets::VertexCustom({ XMFLOAT3(+10.0f, +10.0f, -10.0f), XMFLOAT4(Colors::Red) }),
			SampleAssets::VertexCustom({ XMFLOAT3(+10.0f, -10.0f, -10.0f), XMFLOAT4(Colors::Green) })
		};
		...
		const UINT quadVertexBufferSize = (UINT)vertices.size() * sizeof(SampleAssets::VertexCustom);
		const UINT quadIndexBufferSize = sizeof(indices);


		//START UPLOAD vertices and indices
		...
		ThrowIfFailed(m_device->CreateCommittedResource(
			...
			D3D12_RESOURCE_STATE_COPY_DEST,
			nullptr,
			IID_PPV_ARGS(&m_vertexBufferQuad)));
		...
		ThrowIfFailed(m_device->CreateCommittedResource(
			...
			D3D12_RESOURCE_STATE_GENERIC_READ,
			nullptr,
			IID_PPV_ARGS(&vertexBufferQuadsUploadHeap)));

		NAME_D3D12_OBJECT(m_vertexBufferQuad);
		...
		vertexDataSubresource.pData = vertices.data();
		vertexDataSubresource.RowPitch = quadVertexBufferSize;
		vertexDataSubresource.SlicePitch = vertexDataSubresource.RowPitch;

		UpdateSubresources<1>(m_commandList.Get(), m_vertexBufferQuad.Get(), vertexBufferQuadsUploadHeap.Get(), 0, 0, 1, &vertexDataSubresource);
		m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_vertexBufferQuad.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER));

		//// Initialize the vertex buffer view.
		m_vertexBufferViewQuad.BufferLocation = m_vertexBufferQuad->GetGPUVirtualAddress();
		m_vertexBufferViewQuad.StrideInBytes = SampleAssets::StandardVertexStride;
		m_vertexBufferViewQuad.SizeInBytes = quadVertexBufferSize;
		...
		ThrowIfFailed(m_device->CreateCommittedResource(
			...
			D3D12_RESOURCE_STATE_COPY_DEST,
			nullptr,
			IID_PPV_ARGS(&m_indexBufferQuad)));
		...
		ThrowIfFailed(m_device->CreateCommittedResource(
			...
			D3D12_RESOURCE_STATE_GENERIC_READ,
			nullptr,
			IID_PPV_ARGS(&indexBufferQuadsUploadHeap)));

		NAME_D3D12_OBJECT(m_indexBufferQuad);
	 ...
	 // from the upload heap to the index buffer.
		...
		indexDataSubresource.RowPitch = quadIndexBufferSize;
		indexDataSubresource.SlicePitch = indexDataSubresource.RowPitch;

		UpdateSubresources<1>(m_commandList.Get(), m_indexBufferQuad.Get(), indexBufferQuadsUploadHeap.Get(), 0, 0, 1, &indexDataSubresource);
		m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_indexBufferQuad.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_INDEX_BUFFER));

		// Describe the index buffer view.
		m_indexBufferViewQuad.BufferLocation = m_indexBufferQuad->GetGPUVirtualAddress();
		m_indexBufferViewQuad.Format = SampleAssets::StandardIndexFormat;
		m_indexBufferViewQuad.SizeInBytes = quadIndexBufferSize;

		m_numIndicesQuad = indices.size();
		//END upload vertices and indices
	}
...
void D3D12Bundles::OnUpdate()
{
	...
	m_Rasterizer->SetConstantBufferData(*m_pCurrentFrameResource->m_pConstantBuffers);
}
...
void D3D12Bundles::CreateFrameResources()
{
	...
	for (UINT i = 0; i < FrameCount; i++)
	{
		...
		if (UseCompiuteRasterizer)
		{
			pFrameResource->InitBundle(m_device.Get(), m_pipelineState1.Get(), m_pipelineState2.Get(), i, m_numIndicesQuad, &m_indexBufferViewQuad,
...
void D3D12Bundles::CreateQuadVertexAndIndexBuffer()
{
	//create quad vertex buffer
	//if(UseCompiuteRasterizer)
	{
		...
		static std::array<SampleAssets::VertexCustom, 4> vertices =
		{
			SampleAssets::VertexCustom({ XMFLOAT3(-1.0f, -1.0f, -1.0f), XMFLOAT4(Colors::White) }),
			SampleAssets::VertexCustom({ XMFLOAT3(-1.0f, +1.0f, -1.0f), XMFLOAT4(Colors::Black) }),
			SampleAssets::VertexCustom({ XMFLOAT3(+1.0f, +1.0f, -1.0f), XMFLOAT4(Colors::Red) }),
			SampleAssets::VertexCustom({ XMFLOAT3(+1.0f, -1.0f, -1.0f), XMFLOAT4(Colors::Green) })
		};
	...
	else
	{
		...
		if (UseCompiuteRasterizer)
		{
			...
			m_Rasterizer->Execute(m_commandList.Get(),
				...
				m_vertexBuffer.Get(), m_indexBuffer.Get(), geomentyIndices);


			//Copy compute shader output to back buffer
			...
			m_commandList->CopyResource(GetCurrentBackBuffer(), m_Rasterizer->Output());
